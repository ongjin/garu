...
기존 한국어 형태소 분석기는 대부분 수십 MB의 모델을 서버나 네이티브 런타임으로 구동하는 것을 전제로 합니다. Garu는 **1MB 모델과 391KB WASM 엔진**만으로 브라우저에서 직접 실행되며, 서버 통신 없이 완전한 오프라인 형태소 분석을 수행합니다.
...
- **초경량** -- 1MB 코드북 모델, 391KB WASM (npm 패키지에 포함)
- **높은 정확도** -- F1 95.4% (9,000문장 v15k 골드 테스트셋, ep_norm 정규화), NIKL MP F1 93.9%
...
|  | 서버 기반 분석기 | Garu |
|---|---|---|
| 모델 크기 | 수십 MB | **1MB** (+391KB WASM) |
| 실행 환경 | 서버/네이티브 | **브라우저 (WASM)** |
| 모델 번들 | 별도 로드 | **npm에 포함** |
| 네트워크 | 필요 (API 호출) | **불필요 (오프라인)** |
| 모바일 웹 | 비실용적 | **지원** |

> **정확도**: 9,000문장 v15k 골드 테스트셋 F1 95.4% (ep_norm 정규화 기준), NIKL MP F1 93.9%. 서버 기반 분석기와 다른 설계 목표(경량·브라우저)를 가지므로 단순 수치 비교보다는 사용 환경에 맞는 선택을 권장합니다.
...
**Garu(가루)는 무엇인가요?**
Garu는 1MB 모델과 391KB WASM 엔진만으로 브라우저에서 직접 실행되는 한국어 형태소 분석기입니다. 서버나 네트워크 없이 클라이언트에서 완전 오프라인으로 동작합니다.

**모델과 엔진 크기는 얼마인가요?**
모델은 brotli 압축 기준 1MB, WASM 엔진은 raw 391KB(gzip 169KB)입니다. 모델이 npm 패키지에 포함되어 있어 별도 CDN이나 다운로드가 필요 없습니다.

**정확도는 어느 정도인가요?**
9,000문장 v15k 골드 테스트셋에서 F1 95.4%(ep_norm 정규화 기준), 모두의 말뭉치(NIKL MP)에서 F1 93.9%입니다.
