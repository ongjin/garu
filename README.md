...
- **초경량 모델** -- 2.0MB (gzip ~850KB), 모바일 웹에서도 즉시 로드
- **서버 불필요** -- WebAssembly로 브라우저에서 직접 실행
- **신경망 없음** -- 코드북 + Trigram Viterbi, 학습 파라미터 0개
- **높은 정확도** -- F1 90.9% (NIKL MP 기준, Kiwi 87.9% 상회)
...
|  | Kiwi | MeCab-ko | Garu |
|---|---|---|---|
| 모델 크기 | ~40MB | ~50MB | **2.0MB** |
| 네트워크 전송 | ~15MB | N/A | **~850KB** (gzip) |
| F1 (NIKL MP) | 87.9% | ~85% | **90.9%** |
...
입력 텍스트에 대해 FST 사전(다중 POS 지원)과 접미사 코드북으로 래티스를 구축하고, 희소 트라이그램 비용 테이블 + 단어 수준 bigram 보정을 이용한 Viterbi 디코딩으로 최적 경로를 선택합니다.
