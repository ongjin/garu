...
기존의 한국어 형태소 분석기(Kiwi, MeCab-ko, Komoran 등)는 모두 서버 환경을 전제로 설계되어, 20~50MB의 모델과 네이티브 런타임이 필요했습니다. Garu는 **1.7MB 모델과 174KB WASM 엔진**만으로 브라우저에서 직접 실행되며, 서버 통신 없이 완전한 오프라인 형태소 분석을 수행합니다.
...
- **초경량** -- 1.7MB 모델 (코드북 1.2MB + CNN 526KB, npm 패키지에 포함)
- **높은 정확도** -- F1 91.0% (5,000문장 수동 검증 골드 테스트셋 기준)
- **서버 불필요** -- WebAssembly로 클라이언트에서 직접 실행
- **하이브리드** -- 코드북 + Trigram Viterbi + CNN 재순위, 경량 신경망으로 문맥 이해
...
기존 형태소 분석기(Kiwi, MeCab-ko 등)는 서버 환경에 최적화되어 있어 40~50MB의 모델과 네이티브 런타임이 필요합니다. Garu는 **브라우저에서 직접 실행**되도록 설계된 유일한 한국어 형태소 분석기입니다.
...
> **정확도**: 5,000문장 골드 테스트셋 F1 91.0%, NIKL MP F1 93.6%. 서버 기반 분석기와 다른 설계 목표(경량·브라우저)를 가지므로 단순 수치 비교보다는 사용 환경에 맞는 선택을 권장합니다.
...
└──────────────────┬──────────────────────┘
         ...
         └─────────┬──────────┘
                   ↓
         ┌─────────┴──────────┐
         │  CNN 재순위 (옵셔널) │  2-layer 1D CNN (526KB int8)
...
전체 문장에 대해 래티스를 구축하고, 어절 캐시 항목을 저비용 후보 아크로 주입한 뒤 문장 수준 Viterbi 디코딩을 수행합니다. 디코딩 결과에는 후처리 규칙과 옵셔널 CNN 재순위가 적용됩니다. CNN은 "나는 하늘을 나는 새"에서 첫 번째 "나는"을 NP+JX(대명사), 두 번째를 VV+ETM(날다)으로 정확히 구분하고, "피곤하다"의 하/XSA(형용사파생)와 "공부하다"의 하/XSV(동사파생)를 문맥으로 판별합니다.

코드북 데이터는 모두의 말뭉치(NIKL MP) 골드 어노테이션과 Kiwi 분석기 출력의 하이브리드로 추출합니다. CNN은 NIKL MP 338K 문장에서 음절 수준 BIO 태깅으로 학습되었습니다. 자세한 연구 경과는 [기술 논문](docs/paper.md)을 참고하세요.
