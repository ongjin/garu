...
기존의 한국어 형태소 분석기(Kiwi, MeCab-ko, Komoran 등)는 모두 서버 환경을 전제로 설계되어, 20~50MB의 모델과 네이티브 런타임이 필요했습니다. Garu는 **1.6MB 모델과 174KB WASM 엔진**만으로 브라우저에서 직접 실행되며, 서버 통신 없이 완전한 오프라인 형태소 분석을 수행합니다.
...
- **초경량** -- 1.6MB 모델 (코드북 1.2MB + CNN 408KB, npm 패키지에 포함)
- **높은 정확도** -- F1 91.1% (5,000문장 수동 검증 골드 테스트셋 기준)
...
> **정확도**: 5,000문장 골드 테스트셋 F1 91.1%, NIKL MP F1 93.7%. 서버 기반 분석기와 다른 설계 목표(경량·브라우저)를 가지므로 단순 수치 비교보다는 사용 환경에 맞는 선택을 권장합니다.
