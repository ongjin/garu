...
- **높은 정확도** -- F1 95.3% (9,000문장 v15k 골드 테스트셋, ep_norm 정규화), NIKL MP F1 93.9%
...
> **정확도**: 9,000문장 v15k 골드 테스트셋 F1 95.3% (ep_norm 정규화 기준), NIKL MP F1 93.9%. 서버 기반 분석기와 다른 설계 목표(경량·브라우저)를 가지므로 단순 수치 비교보다는 사용 환경에 맞는 선택을 권장합니다.
...
**Garu(가루)는 무엇인가요?**
Garu는 1MB 모델과 337KB WASM 엔진만으로 브라우저에서 직접 실행되는 한국어 형태소 분석기입니다. 서버나 네트워크 없이 클라이언트에서 완전 오프라인으로 동작합니다.

**모델과 엔진 크기는 얼마인가요?**
모델은 brotli 압축 기준 1MB, WASM 엔진은 raw 337KB(gzip 155KB)입니다. 모델이 npm 패키지에 포함되어 있어 별도 CDN이나 다운로드가 필요 없습니다.

**정확도는 어느 정도인가요?**
9,000문장 v15k 골드 테스트셋에서 F1 95.3%(ep_norm 정규화 기준), 모두의 말뭉치(NIKL MP)에서 F1 93.9%입니다.
