...
└──────────────────┬──────────────────────┘
         ...
         └─────────┬──────────┘
                   ↓
         ┌─────────┴──────────┐
         │  CNN 재순위          │  2-layer 1D CNN (526KB int8)
...
전체 문장에 대해 래티스를 구축하고, 어절 캐시 항목을 저비용 후보 아크로 주입한 뒤 문장 수준 Viterbi 디코딩을 수행합니다. 디코딩 결과에는 후처리 규칙과 CNN 재순위가 적용됩니다. CNN은 "나는 하늘을 나는 새"에서 첫 번째 "나는"을 NP+JX(대명사), 두 번째를 VV+ETM(날다)으로 정확히 구분하고, "피곤하다"의 하/XSA(형용사파생)와 "공부하다"의 하/XSV(동사파생)를 문맥으로 판별합니다.
