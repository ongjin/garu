...
33. **Lossless 속도 스택** (2026-07-02) → **2.35× 가속** (WASM 0.62→0.26ms/sent), Kiwi 격차 5.8×→2.4×. opt-level z→3 + wasm-opt -Oz→-O3(WASM raw 341→391KB, brotli +9KB) + inline FxHash(SipHash 18% 제거) + word_bigram 조회 arc당 1회 hoist. 9,000문장 출력 byte-identical + F1 0.9544 불변 검증. 잔여 lossless 후보(DP dense화·할당 churn, ~3.0×까지)는 미착수.
