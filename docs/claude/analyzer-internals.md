...
- **표면 환각(정합성 버그)**: 접미사 코드북에 키 표면 ≠ 재구성 표면인 **절단 엔트리**가 있음 (예: 키 `배`→`[배우/VV,었/EP]`(배웠), `표`→`[표,하,었]`(표했), `시장`→`[시,장악,하,었]`). Strategy C가 이를 `span=키길이` 아크로 주입해 출력 표면이 입력과 불일치(`2.2배다.`→`배출/하/았/다`). 가드 추가: stem 접두(첫 어미 전 = 내용+하/되 XSV·계사 이)의 표면 길이 > span이면 스킵(codebook.rs Strategy C, commit b5b4989). **주의: 같은 절단 엔트리가 A2/A3 접미사 lookup에도 누출**됨 — 현재 골드 출력엔 미표출(1위 안 됨)이나 잠재. 근본 해결은 코드북 빌드/로드에서 재구성≠키 엔트리 제거. dump_arcs로 `[i..i+1]`인데 형태소가 다음절이면 이 클래스.
