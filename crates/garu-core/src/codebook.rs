...
/// 정직하다, 깨끗하다, 조용하다 etc.
const ADJ_ROOTS: &[&str] = &[
    "가난", "가능", "가혹", "각별", "간단", "간편", "강건", "강경", "강력", "거대",
    "건강", "건전", "건조", "걸출", "겸손", "고독", "고요", "고유", "곤란", "공정",
    "공평", "과도", "광대", "광범위", "괴상", "궁금", "규칙", "균일", "급속", "깔끔",
    "깨끗", "난폭", "냉정", "다양", "다정", "단순", "단일", "당연", "대담", "대등",
    "독특", "동등", "동일", "랜덤", "막강", "만족", "무궁무진", "무능", "무리", "무식",
    "무질서", "무한", "미세", "미안", "미흡", "밀접", "바람직", "복잡", "부당", "부실",
    "부유", "부정", "부족", "부지런", "부진", "분명", "분별", "분주", "불리", "불안",
    "불우", "불편", "불필요", "불행", "비옥", "비참", "빈약", "사악", "상당", "선량",
    "선명", "성실", "솔직", "순수", "스무드", "시급", "시원", "신기", "신비", "신선",
    "신성", "신속", "신중", "심각", "심심", "쌀쌀", "악랄", "안녕", "안전", "안정",
    "암울", "양호", "억울", "영원", "온난", "완벽", "완전", "왕성", "우세", "우수",
    "우연", "우울", "우월", "원활", "위험", "유능", "유력", "유리", "유망", "유명",
    "유사", "유용", "유의미", "유익", "유일", "유한", "유해", "유효", "음란", "이상",
    "일정", "자비", "잔인", "잔혹", "저명", "저조", "적막", "적법", "적정", "적합",
    "전무", "정밀", "정직", "정확", "조용", "주요", "중대", "중요", "진실", "창피",
    "천박", "철저", "청렴", "충만", "충실", "취약", "치열", "친밀", "친숙", "친절",
    "침착", "태연", "투명", "특별", "특수", "특이", "특정", "편리", "편안", "평등",
    "평온", "평탄", "포멀", "풍성", "피곤", "필요", "핍진", "행복", "허무", "현란",
    "현명",
];
...
impl CodebookAnalyzer {
    ...
    fn fix_vx(tokens: &mut [Token]) {
        for i in 1..tokens.len() {
            ...
            if cur_pos == Pos::VV {
                match form {
                    "하" => {
                        // 지/고 하다, 그리고 의무 보조용언 어야/아야 + 하다 (…해야 한다)
                        if prev_form == "지" || prev_form == "고"
                            || prev_form == "어야" || prev_form == "아야" {
                            tokens[i].pos = Pos::VX;
                        }
                    }
    ...
    /// 시/NNG → NNB: 의존명사 "~할 때" (추론 시, 학습 시, 종료 시). 명사/XSN 뒤에서만.
    /// 골드 시/NNB 87% (40:6). 숫자(SN) 뒤 시각의 시는 prev가 NNG/XSN이 아니라 미발동.
    fn fix_si_dependent_noun(tokens: &mut [Token]) {
        for i in 1..tokens.len() {
            if tokens[i].text == "시"
                && tokens[i].pos == Pos::NNG
                && matches!(tokens[i - 1].pos, Pos::NNG | Pos::XSN)
            {
                tokens[i].pos = Pos::NNB;
            }
        }
